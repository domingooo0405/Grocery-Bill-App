...
import { addItem, getItemById, updateItem } from "../../service/ItemService";
...
class AddItemComponent extends Component {
  ...
  componentDidMount() {
    if (this.state.id === "_add") {
      ...
      getItemById(this.state.id).then((response) => {
  ...
  addOrUpdate = (event) => {
    ...
    if (
      ...
      Message.innerHTML = "<h4 class='message'> Please complete details!<h4/>";
      ...
      if (this.state.id === "_add") {
        ...
        if (DiscountPercentage > 100) {
          Message.innerHTML =
            "<h4 class='message'>Not more than 100% discount!<h4/> ";

          setTimeout(() => document.querySelector(".message ").remove(), 3000);
        } else if (DiscountPercentage < 0) {
          Message.innerHTML =
            "<h4 class='message'>Not less than 0% discount!<h4/> ";

          setTimeout(() => document.querySelector(".message").remove(), 3000);
        } else if (Price <= 0) {
          Message.innerHTML =
            "<h4 class='message'>Not less than or equal 0 price!<h4/> ";
        ...
        if (
          ...
          Message.innerHTML =
            "<h4 class='message'> Please complete details!<h4/>";
          ...
          updateItem(item, this.state.id).then((response) => {
            this.props.history.push("/items");
          });
        }
      }
  ...
  getTitle() {
    if (this.state.id === "_add") {
      return <h1 style={{ fontSize: "30px" }}> Add Item</h1>;
    } else {
      return <h1 style={{ fontSize: "30px" }}> Update Item</h1>;
    }
  }
  render() {
    return (
